...
class NLK_Env(gym.Env):
    def __init__(self, reward_design_1 = 5, reward_design_2 = 5, deterministic = False):
        ...
        # Observation space contains round, x/px means in cm and histogram of structure
        self.observation_space = spaces.Dict(
                                    spaces = {
                                        "round": gym.spaces.Box(0, 10, (1,), dtype=np.float32),
    ...
    def reset(self, seed = None, options = None):
        self.round = np.float32(np.array([0])) #round set to 0
        self.activated_NLK = False
        ...
        if options is None:
            ...
            if "x,px" in options.keys():
                ...
                #generate new x/px values and normalize them
                x_points = norm.rvs(loc=self.x, scale = self.sigma_x, size = 1000) * self.x_normalization  
                px_points = norm.rvs(loc=self.px, scale=self.sigma_px, size = 1000) * self.px_normalization  
            elif "points" in options.keys():
    ...
    def step(self, action): 
        ...
        if self.round[0]*self.round_normalization == 1000:
            #if electrons survived 1000 rounds we are finished
            xpx_mean, image = encoder(self.points, self.x_normalization, self.px_normalization)
            
            out_dict={"round": self.round,
                  "x,px mean": xpx_mean,
                  "image": image}
            reward = 1000/self.reward_normalization
            truncated = False
            terminated = True
            info = {}
        ...
        self.round += np.float32(1/self.round_normalization)
        ...
        if NLK_activated==False:
            ...
            if check != False: 
                #print("Lost all points in the septum")
                return check
            ...
            if self.check_if_empty() != False: 
                reward = (self.reward_design_1-self.reward_design_2*np.abs(NLK_strength))/self.reward_normalization
